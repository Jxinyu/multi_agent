...
class EnterpriseLocalVLMParser:
    ...
    def _process_single_image(self, image_input: str | bytes) -> str:
        """调用 Ollama 进行单图推理"""
        ocr_prompt = (
            "必须使用中文回答。"
        ...
        try:
            ...
            raise DocumentParsingError("本地视觉模型推理失败") from e
    ...
    async def parse_file(self, file_path: str) -> str:
        ...
        logger.info("开始 Qwen2.5-VL 本地解析")
        ...
        try:
            ...
            if not full_text.strip():
                raise DocumentParsingError("本地视觉模型未返回解析内容")

            logger.info("Qwen2.5-VL 本地解析完成，耗时 %.1fs", time.time() - start_time)
            return full_text

        except FileNotFoundError as fe:
            logger.error("待解析文档不存在")
            raise DocumentParsingError(f"文档不存在：{str(fe)}") from fe
        except ValueError as ve:
            logger.error("待解析文档校验失败")
