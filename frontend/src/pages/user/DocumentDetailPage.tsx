...
export function DocumentDetailPage({ mode }: DocumentDetailPageProps) {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      setDetail(await (mode === 'enterprise' ? fetchKnowledgeBaseDocumentDetail(documentId) : fetchUserDocument(documentId)));
    } catch (reason) {
  ...
  const ingest = async () => {
    ...
    setError('');
    try {
      await ingestKnowledgeBaseDocument(documentId, 'graphrag');
      await load();
    } catch (reason) {
