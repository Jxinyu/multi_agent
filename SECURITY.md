...
- `audit:read` 应与知识库管理权限分离，只授予安全审计角色。
- 审计事件按租户查询，记录 actor、action、resource、outcome、request ID 和非敏感元数据，不记录提示词、正文、文件名、路径或令牌。
- PostgreSQL 触发器阻止普通连接修改或删除审计行，但不能防止数据库超级用户、基础设施管理员或备份系统篡改。
- 高合规环境必须将事件外送到 SIEM 或不可变存储，并由企业确定保留期、访问审批、完整性证明和事件响应流程。
